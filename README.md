...
- **Share an outcome with context?** Choose **Share this view** to preview the current human input and agent response, then save the view locally. Hover over content to select particular messages or passages, then add a title or annotation. Saved views can be reopened and edited later. Whole-session publishing is being redesigned; excerpt publishing awaits hosted integration. See the [excerpt contract and limits](docs/share-excerpt-v1.md).
